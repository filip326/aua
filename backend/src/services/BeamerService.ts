...
class BeamerService {
    ...
    public sendRS232Command(command: string) {
    ...
    public async sendRequest(conf: Omit<Omit<BeamerRequest, "port">, "host">): Promise<ParsedBeamerReponse> {
        this.initializeTCP();
        const req: BeamerRequest = {
            ...conf,
            host: this.host,
            port: this.port,
        };
